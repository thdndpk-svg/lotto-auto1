...
def build_summary_page(metrics: KnowledgeMetrics, today: str) -> str:
    ...
    body.append("")
    body.append("## 피드백 학습")
    body.append("- [[feedback-learning-summary]]")
    body.append("- [[feedback-latest-result]]")
    return page(
        {
            "type": "source",
            "created": today,
...
"""
    ...
    if (wiki_dir / "feedback-learning-summary.md").exists():
        entries.append(("note", "feedback-learning-summary", "미당첨 원인과 적중요인 누적 학습", today))
    if (wiki_dir / "feedback-latest-result.md").exists():
        entries.append(("note", "feedback-latest-result", "최근 추천 결과 오답노트", today))
